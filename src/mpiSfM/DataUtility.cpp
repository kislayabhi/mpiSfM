...
void LoadKeys(string image_filepath, vector<string>& image_filenames)
{
	...
	try
	{
		if (exists(p)) {
			if (is_directory(p)) {
				cout << p << " Directory Containing Keys Found:\n";
				for (boost::filesystem::directory_entry& x : boost::filesystem::directory_iterator(p)) {
					string file_string = x.path().filename().string();
					if (file_string.substr(file_string.size() - 4, 4) == ".key")
...
/* Loads image names as strings from a directory*/
void LoadImages(string image_filepath, vector<string>& image_filenames)
{
	boost::filesystem::path p(image_filepath.c_str());
	...
	try
	{
		if (exists(p)) {
			if (is_directory(p)) {
				...
				for (boost::filesystem::directory_entry& x : boost::filesystem::directory_iterator(p)) {
					string file_string = x.path().filename().string();
					if (file_string.substr(file_string.size() - 4, 4) == ".bmp")
						image_filenames.push_back(x.path().filename().string());
				}
				sort(image_filenames.begin(), image_filenames.end());
			}else
